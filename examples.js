function randomStreamObservable(slowDownFactor, name, values) {
    if (slowDownFactor === void 0) { slowDownFactor = 1; }
    if (name === void 0) { name = 'shapes$'; }
    if (values === void 0) { values = ['□', '△', '○', '▷', '☆']; }
    return new Observable(randomStreamProducer(slowDownFactor, values), name);
}
function randomStreamProducer(slowDownFactor, values) {
    // ['□', '△', '○', '▷', '☆', '■', '▲', '●', '▶', '★']
    if (slowDownFactor === void 0) { slowDownFactor = 1; }
    if (values === void 0) { values = ['□', '△', '○', '▷', '☆']; }
    return function (observer) {
        ...
        function randomLoop() {
            ...
            cancelationToken = setTimeout(function () {
                if (!isPaused)
...
function combineShapeAndFill(fill, shape) {
    ...
    var filledShapes = ['■', '▲', '●', '▶', '★'];
    var isFilled = function (val) { return filledShapes.indexOf(val) > -1; };
    var getShapeIndex = function (val) {
...
var examples = {
    ...
    'filter': {
        ...
        exec: function (done) {
            return randomStreamObservable()
                .filter(function (v) { return filterTriangles(v, false); })
                .subscribe({ complete: done });
        },
    ...
    'stopwatch-start-pauze': {
        ...
        exec: function () {
            ...
            startEl.innerText = '►';
            valueEl.innerText = '0';
            var updatePauseButton = function (isOn) {
